...
func FetchFrankfurterRate(date time.Time) (decimal.Decimal, time.Time, error) {
	...
	zarValue, ok := result.Rates["ZAR"]
	if !ok {
		return decimal.Zero, time.Time{}, fmt.Errorf("ZAR rate not found in response")
	}

	var rateStr string
	switch v := zarValue.(type) {
	case string:
		rateStr = v
	case float64:
		rateStr = fmt.Sprintf("%.6f", v)
	default:
		return decimal.Zero, time.Time{}, fmt.Errorf("unexpected ZAR rate type: %T", zarValue)
	}
