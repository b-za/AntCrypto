...
func FetchAllRatesForDate(date time.Time, coins []string, cache *RateCache, maxFallbackDays int) (map[string]CoinRate, ZARExchangeRate, []error) {
	...
	for _, coin := range coins {
		...
		if exists {
			rates[coin] = CoinRate{
				ZARPerUnit: cached.Value.Mul(usdToZARRate),
...
func CalculateUnitsForZAR(zarValue decimal.Decimal, coinRates map[string]CoinRate) map[string]decimal.Decimal {
	units := make(map[string]decimal.Decimal)

	for coin, rate := range coinRates {
		units[coin] = ConvertZARToCrypto(zarValue, rate.ZARPerUnit)
	}
